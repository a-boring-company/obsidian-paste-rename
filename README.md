...
The successor uses repository slug `obsidian-paste-rename`, plugin ID `paste-rename`, and display name **Paste Rename**. Version 2.0.1 requires Obsidian 1.1.1 or newer and fixes figure-only conversion when Obsidian updates the active editor before the vault write resolves, while historic note-relative figures remain unmigrated. It is not yet listed in Obsidian's Community Plugins directory.
